...
const uint8_t group_1_1[] = {3,4,5};
const uint8_t group_1_2[] = {2,6,7};
const uint8_t group_1_3[] = {1,8,9};

const uint8_t group_2_1[] = {12,13,14,15};
const uint8_t group_2_2[] = {10,11,16};
const uint8_t group_2_3[] = {9,17,18};

const uint8_t group_3_1[] = {22,23,24};
const uint8_t group_3_2[] = {21,25,26};
const uint8_t group_3_3[] = {19,20,27};
...
uint32_t getGridColorValue(NamedColors color) {
    switch (color) {
        case COLOR_RED:
            return strip.Color(255, 0, 0);
        case NamedColors::COLOR_GREEN:
...
void setGridColor(int sectionRow, int sectionCol, NamedColors color) {
    ...
    // STUPID hack to vertically mirror LED GRiD
    sectionCol = 3 - sectionCol - 1;
    ...
    uint32_t packedColor = getGridColorValue(color);
    ...
    for (int y = startY; y < endY; y++) {
        for (int x = startX; x < endX; x++) {
            ...
            grid.setPixelColor(i, packedColor);
        }
    }
...
void fillGrid(NamedColors color){
    ...
    grid.show();  // Only call once, after filling all pixels
}
...
void rainbowFade(int wait, int rainbowLoops) {
  ...
  for(uint32_t firstPixelHue = 0; firstPixelHue < rainbowLoops*65536;
    ...
    for(int i=0; i<grid.numPixels(); i++) { // For each pixel in strip...
      ...
      // Here we're using just the three-argument variant, though the
      // second value (saturation) is a constant 255.
      grid.setPixelColor(i, strip.gamma32(strip.ColorHSV(pixelHue, 255,
        255 * fadeVal / fadeMax)));
